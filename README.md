# Service Bus Explorer — Power Apps Code App

A React + TypeScript + Vite app deployed as a [Power Apps Code App](https://learn.microsoft.com/power-apps/maker/canvas-apps/code-apps-overview), providing a web UI to interact with Azure Service Bus queues.

## Features

- **Send messages** to a queue (auto base64-encoded)
- **Peek all messages** with peek-lock from Main or Dead Letter queues (batched fetch, no cap)
- **Table view** matching Azure Portal headers — Sequence Number, Message ID, State, Body Size, Content Type, Label/Subject, Correlation ID, Session ID, and decoded Message Text
- **Dead Letter columns** — Dead Letter Reason and DL Error shown when viewing DLQ
- **Complete** messages (permanently removes from queue) with confirmation dialog
- **Dead-letter** messages (moves to Dead Letter sub-queue) with confirmation dialog
- **Abandon** messages (releases lock, returns to queue)
- **Unlock buttons** — release all locks in parallel and clear the table, per queue
- **Auto-refresh** — re-peeks the active queue after sending a message
- **Per-message loading** — action target fades while the operation runs
- **Parallel lock release** — uses `Promise.allSettled` for fast bulk unlock
- **Client-side pagination** with numbered page buttons and ellipsis gaps
- **Loading banner** with descriptive progress (connecting, releasing locks, fetching)
- **Base64 decoder** utility panel
- **Dark glass-panel UI** with colour-coded status bar
- **Configurable queue name** — set via `VITE_QUEUE_NAME` env var, editable in the header at runtime, persisted to localStorage
- **Session ID support** — optional session ID input in the header for session-enabled queues/topics; passed to peek, complete, abandon, dead-letter, and send operations; persisted to localStorage
- **Theme picker** — Dark, Light, Midnight, and Nord themes with localStorage persistence
- **Responsive design** — tablet (≤768px) and mobile (≤480px) breakpoints
- **Full-width layout** — uses all available screen space

## Prerequisites

- Node.js 18+
- npm
- [Power Platform CLI (`pac`)](https://learn.microsoft.com/power-platform/developer/cli/introduction)
- A Power Platform environment with Code Apps enabled
- An Azure Service Bus namespace with Entra ID authentication enabled

## Setup

1. **Clone and install**
   ```bash
   git clone https://github.com/Griffy42/CodeApps-Power.git
   cd CodeApps-Power
   npm install
   ```

2. **Authenticate with Power Platform**
   ```bash
   pac auth create
   pac env select --environment "<your-environment-id>"
   ```

3. **Initialise the Code App** (creates `power.config.json`)
   ```bash
   npx power-apps init
   ```

4. **Create a Service Bus connection** in [make.powerapps.com](https://make.powerapps.com):
   - Go to **Connections** → **+ New connection** → **Service Bus** → **Microsoft Entra ID** auth
   - Enter your namespace endpoint (e.g. `sb://yournamespace.servicebus.windows.net`)

5. **Link the data source**
   ```bash
   npx power-apps add-data-source --api-id shared_servicebus
   ```
   - Select **No** for connection reference
   - Paste the connection ID from the connection URL in make.powerapps.com

6. **Configure the queue name** — create a `.env` file in the project root:
   ```env
   VITE_QUEUE_NAME=your-queue-name
   ```
   The queue name can also be changed at runtime via the input field in the app header. The value is persisted to localStorage.

7. **Build and deploy**
   ```bash
   npm run build
   npx power-apps push
   ```

## Dev Commands

| Command | Description |
|---------|-------------|
| `npm install` | Install dependencies |
| `npm run dev` | Run locally with Vite dev server |
| `npm run build` | Production build |
| `npm run lint` | Lint with ESLint |
| `npx power-apps run` | Run locally with Power Apps context |
| `npx power-apps push` | Deploy to Power Platform |

## RBAC

The signed-in user needs **Azure Service Bus Data Owner** (or Data Sender + Data Receiver) role on the Service Bus namespace for the Entra ID connector to work.

## Sharing with Other Users

The app uses a **Connection Reference** for the Service Bus connector, so end users don't need to manually enter the namespace endpoint. When shared:

- Users are prompted to **sign in** to the existing connection (Entra ID) — no namespace URL required
- Ensure each user has the required **RBAC role** (see above) on the Service Bus namespace
- To share the app: go to [make.powerapps.com](https://make.powerapps.com) → **Apps** → select the app → **Share** → add users or groups

## Project Structure

```
src/
  App.tsx          — Main component (send, peek-all, complete, dead-letter, abandon,
                     unlock, table view, pagination, confirmation dialogs,
                     per-message loading, parallel lock release, theme picker)
  App.css          — Themed glass-panel UI, table layout, queue toggle with unlock
                     buttons, responsive breakpoints (tablet + mobile)
  index.css        — Global base styles, theme token definitions
                     (dark, light, midnight, nord)
  main.tsx         — React entry point
  generated/       — Auto-generated by Power Apps CLI (do not edit)
    services/ServiceBusService.ts  — Service Bus connector SDK
    models/ServiceBusModel.ts      — TypeScript types
```

## Notes

- `power.config.json` is gitignored — it contains environment-specific IDs. Each developer creates their own via `npx power-apps init`.
- The `src/generated/` folder is auto-generated by the Power Apps CLI when you run `add-data-source`. Do not edit manually.
- SAS key authentication may be disabled by Azure Policy on some namespaces — use Entra ID auth in that case.

## Changelog

### Configurable queue name (April 2026)

- **Refactor:** Queue name is no longer hardcoded in `App.tsx`; defaults from `VITE_QUEUE_NAME` env var
- **Feature:** Editable queue name input in the header — change queues without redeploying
- **Persistence:** Selected queue name saved to localStorage across sessions
- **Added:** `.env` file (gitignored) for local configuration

### Code review fixes (April 2026)

- **Bug fix:** Peek abandon path now uses the `queueType` parameter instead of stale `activeQueueType` state
- **Bug fix:** `unlockMessages` now inspects `Promise.allSettled` results and reports individual failures instead of a dead `catch` block
- **Bug fix:** Status colour for "Released" and "Seeded" messages now correctly shows green (success) instead of blue (info)
- **Bug fix:** Empty-state display uses a `hasPeeked` boolean flag instead of fragile `status.includes('Got')` string check
- **Cleanup:** Removed ~30 lines of dead CSS selectors (`.message-card`, `.message-header`, etc.) from the mobile breakpoint
- **Comment fix:** Corrected `unlockMessages` JSDoc (was describing peeking instead of releasing locks)
- **Comment fix:** Changed "card" to "row" in `busyMsgId` comment to match the table-based UI